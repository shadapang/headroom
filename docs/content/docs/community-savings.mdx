---
title: Community Savings
description: Aggregate savings from Headroom instances across the community. Anonymous telemetry data — no prompts, no content, no PII.
---

Real-time aggregate metrics from Headroom proxy instances worldwide. All data is anonymous — only token counts, compression ratios, and cost estimates are collected. Telemetry is off by default; [opt in](https://github.com/headroomlabs-ai/headroom/blob/main/headroom/telemetry/beacon.py) with `HEADROOM_TELEMETRY=on`.

## Overview

<CommunityStatsHeader />

## Savings Over Time

<CommunityCharts section="area" />

## Top Savings by Instance

<CommunityCharts section="bar" />

## Instance Details

<CommunityCharts section="table" />
